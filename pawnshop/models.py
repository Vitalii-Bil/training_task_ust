...
class Good(models.Model):
    ...
    def get_delivery_price(self):
        return f"{decimal.Decimal(round(self.weight * self.material.cost, 2)):.2f}"

    get_delivery_price.fget.short_description = "Ціна здачі станом на сьогодні (грн.)"

    @property
    def get_redemption_price(self):
        redemption_price = round(
            float(self.weight * self.material.cost)
            + float(self.weight * self.material.cost) * float(self.rate_field) * 0.01,
            2,
        )
        return f"{decimal.Decimal(redemption_price):.2f}"
    ...
    def save(self, *args, **kwargs):
        if not self.pk:
            ...
            self.first_redemption_price = round(
                float(self.weight * self.material.cost)
                + float(self.weight * self.material.cost) * float(self.rate_field) * 0.01,
                2,
            )
